$(function(){
    ...
    //$(window).on('load',function() {
        ...
        //$('#top').css({opacity: '0'}).animate({opacity: '1'}, 1000);
    //});

    $(window).ready(function() {
        //$('#top').animate({ opacity: 1 }, { duration: 2000, easing: 'swing'});
    ...
    //メニュー表示slideToggleで実装
    $('.hamb').on('click',function(){
        $('.open').slideToggle(250);
        $('.darkLayer').toggleClass('_on');
    });
    ...
    $(window).on('resize', function() {
        ...
        if(windowWidth > 560) {
            ...
            $('.darkLayer').removeClass('_on');
        }
        else {
            $('.open').css('display','block').hide();
            $('.darkLayer').removeClass('_on');
        }
    });
